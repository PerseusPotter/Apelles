...
void main(void) {
  ...
  vec4 color = texelFetch(sampler, pos, 0);
  vec2 k00 = grayscale(color);
  vec2 k01 = grayscale(texelFetch(sampler, pos + ivec2(+1, +0), 0));
  vec2 k10 = grayscale(texelFetch(sampler, pos + ivec2(+0, +1), 0));
  vec2 k11 = grayscale(texelFetch(sampler, pos + ivec2(+1, +1), 0));

  vec2 Gx = k00 - k11;
  vec2 Gy = k01 - k10;
  vec2 G = sqrt(Gx * Gx + Gy * Gy);
  ...
  fragColor = vec4(color.rgb, color.a * clamp(G[0] + G[1], 0.0, 1.0));
}