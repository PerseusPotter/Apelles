...
object PrimitiveUVInternal : Geometry() {
    override val name = "primitiveuvinternal"
    override fun render(pt: Double, params: List<Double>) {
        ...
        while (iter.hasNext()) {
            ...
            for (i in 0 until 6) pos(iter.next(), iter.next(), iter.next(), iter.next(), iter.next())
        }
        draw()
    }
    ...
    override fun getVertexCount(params: List<Double>): Int = params.size / 34 * 6
    override fun getIndicesCount(params: List<Double>): Int = params.size / 34 * 6
